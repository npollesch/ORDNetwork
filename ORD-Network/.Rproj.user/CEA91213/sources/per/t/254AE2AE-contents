...
create.groups<-function(graph,method,noGroups=NA,all=F,ECurve=F,quiet=T){
GOutC<-list()
GOutAll<-list()
if(ECurve){E(graph)$curved<-F}
...
GOutC[[1]]<-graph
  for(i in 1:length(newEdges)){
    for(j in 1:i){
      GOutC[[i+1]]<-add.edges(GOutC[[i]],newEdges[[j]],curved=ECurve,color=rainbow(length(newEdges))[j])
  ...
  for(j in 1:(sum(groups[,1]))){
    if(j<groups[2,1]+1){
      for(i in 1:(groups[2,2]-1)){
        ...
        film[[i+1]]<-simplify(add.edges(film[[i]],as.vector(combn(cast[[i]],2)),curved=ECurve,color=rainbow(sum(groups[1,]))[j]),edge.attr.comb="first")
  ...
  if(is.null(dim(groups))){
    ...
    for(j in 1:(groups[1])){
        for(i in 1:(groups[2]-1)){
          ...
          film[[i+1]]<-simplify(add.edges(film[[i]],as.vector(combn(cast[[i]],2)),curved=ECurve,color=rainbow(groups[1])[j]),edge.attr.comb="first")
